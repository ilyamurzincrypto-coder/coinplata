...
export function makeLedgerCtx(overrides = {}) {
  ...
  const transactions = [
    ...
    { id: "tx_rent", effectiveDate: "2026-05-05T00:00:00Z", createdAt: "2026-05-05T00:00:00Z", kind: "manual", sourceRefId: null, reversesTransactionId: null, metadata: {} },
  ...
  const entries = [
    ...
    // an expense entry, on its own tx dated 2026-05-05 (effective_date is what period reports use)
    { id: "je8", transactionId: "tx_rent", accountId: "ac_rent_usd", direction: "dr", amount: 1800, currency: "USD", clientId: null, partnerId: null, note: "rent", createdAt: "2026-05-05T00:00:00Z" },
  ];
...
import { assetsByOfficeCurrency } from "./v2selectors.js";
describe("assetsByOfficeCurrency — иерархическое дерево Office → Currency → Account", () => {
  it("строит дерево из asset-счетов; null-офис в конце", () => {
    const ctx = makeLedgerCtx();
    ...
    expect(tree.map((o) => o.officeId)).toEqual(["office-mark", null]);
    expect(tree[0].totalInBase).toBe(11150);
    expect(tree[1].totalInBase).toBe(1000);
  });

  it("валюты внутри офиса отсортированы по |totalInBase| desc; счета внутри валюты — по |balanceInBase| desc", () => {
    const ctx = makeLedgerCtx();
    const tree = assetsByOfficeCurrency(ctx);
    const mark = tree.find((o) => o.officeId === "office-mark");
    expect(mark.currencies.map((c) => c.currency)).toEqual(["USD", "USDT"]); // USD 11000 > USDT 150
    expect(mark.currencies[0].accounts.map((a) => a.accountId)).toEqual(["ac_cash_usd_mark"]);
    expect(mark.currencies[0].total).toBe(11000);
    expect(mark.currencies[0].totalInBase).toBe(11000);
  });

  it("officeFilter=<uuid> исключает null-office", () => {
    const ctx = makeLedgerCtx({ officeFilter: "office-mark" });
    const tree = assetsByOfficeCurrency(ctx);
  ...
  it("null-office sinks to end даже при большем totalInBase", () => {
    const ctx = makeLedgerCtx({
      accounts: [
        { id: "a1", code: "1", name: "small", type: "asset", subtype: "cash", currency: "USD", officeId: "o1" },
        { id: "a2", code: "2", name: "big", type: "asset", subtype: "cash", currency: "USD", officeId: null },
      ],
      balances: [
    ...
    const tree = assetsByOfficeCurrency(ctx);
    expect(tree.map((o) => o.officeId)).toEqual(["o1", null]);
  });
});
