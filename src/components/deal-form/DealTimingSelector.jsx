...
export default function DealTimingSelector({ value = "now", onChange }) {
  return (
    <div className="px-7 py-5 border-b border-border-soft">
      <div className="text-micro text-muted uppercase mb-3">Когда исполнить</div>
        ...
        {OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={`group p-3.5 rounded-card text-left transition-all duration-150 ease-apple border-[1.5px] ${
                active
            ...
            >
              <div
                className={`w-7 h-7 rounded-lg flex items-center justify-center mb-2.5 transition-colors ${
                  active ? "bg-ink text-white" : "bg-surface text-ink-soft"
                }`}
              >
                <Icon className="w-3.5 h-3.5" strokeWidth={2.2} />
              </div>
              <div className="text-body-sm font-semibold text-ink">{opt.title}</div>
              <div className="text-tiny text-muted mt-0.5">{opt.subtitle}</div>
            </button>
          );
        })}
      </div>
