...
export default function RatesSidebar({ currentOffice, onOpenRates, onExpandedChange }) {
  ...
  return (
    <aside className="bg-surface rounded-card p-1.5 flex flex-col h-full">
      ...
      {/* Паст-ввод */}
      <div className="px-2 py-2 mt-1 border-t border-border-soft shrink-0">
        <button
          type="button"
          onClick={() => setPasteOpen(true)}
          className="w-full px-2 py-1.5 rounded-[8px] text-caption font-semibold text-accent bg-accent-bg hover:bg-indigo-100 transition-colors"
        >
          Вставить курсы
        </button>
      </div>
