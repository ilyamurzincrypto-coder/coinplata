  ...
  return (
    <div ref={rootRef} className="relative">
      <div className="flex items-stretch gap-1.5">
        ...
        >
          <Search className="w-3.5 h-3.5 text-muted-soft ml-2.5" />
          <input
            type="text"
            value={query}
          ...
          {query && (
            <button
              ...
              className="p-0.5 rounded hover:bg-surface-sunk text-muted-soft mr-1"
              title="Clear"
            >
              <X className="w-3 h-3" />
            </button>
        ...
        <div className="absolute z-40 mt-1 w-full bg-white border border-border-soft rounded-[10px] shadow-xl shadow-soft py-1 max-h-64 overflow-auto">
          ...
          {results.map((p) => (
            ...
            >
              ...
              {value === p.name && <Check className="w-3 h-3 text-accent" />}
            </button>
          ))}
          {showCreateOption && (
            <div className="border-t border-border-soft mt-1 pt-1 px-1">
              <button
                type="button"
                onClick={() => setShowAdd(true)}
                className="w-full text-left px-2 py-1.5 rounded-[6px] hover:bg-accent-bg flex items-center gap-2 text-accent"
              >
