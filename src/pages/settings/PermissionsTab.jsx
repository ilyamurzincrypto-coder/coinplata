...
const LEVEL_STYLES = {
  disabled: "bg-surface-sunk text-muted",
  view: "bg-info-soft text-info ring-1 ring-sky-200",
  edit: "bg-success-soft text-success ring-1 ring-emerald-200",
};
...
export default function PermissionsTab() {
  ...
  return (
    <>
      ...
      {!isAdmin && (
        <div className="m-5 p-3 bg-warning-soft border border-warning/20 rounded-card text-caption text-warning">
      ...
      <div className="p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-body-sm border-separate border-spacing-0">
            <thead>
              <tr>
                <th className="text-left sticky left-0 bg-white z-10 py-2.5 pr-4 text-tiny font-bold text-muted tracking-[0.1em] uppercase border-b border-border-soft">
                  User
                </th>
                {SECTIONS.map((s) => (
                  <th
                    key={s}
                    className="text-center px-2 py-2.5 text-tiny font-bold text-muted tracking-[0.1em] uppercase border-b border-border-soft whitespace-nowrap"
                  >
                    {t(`section_${s}`)}
                  </th>
                ))}
                <th className="w-10 border-b border-border-soft"></th>
              </tr>
            </thead>
            <tbody>
              {activeUsers.map((u) => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-surface-soft/60">
                    <td className="sticky left-0 bg-white py-2 pr-4 border-b border-border-soft">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-tiny font-semibold ${
                            u.role === "admin"
                        ...
                        <div>
                          <div className="text-body-sm font-semibold text-ink">{u.name}</div>
                          <div className="text-tiny text-muted">{ROLES[u.role]?.label}</div>
                        </div>
                      </div>
                    </td>
                    {SECTIONS.map((s) => {
                      ...
                      return (
                        <td key={s} className="px-2 py-2 border-b border-border-soft">
                          {isRoleAdmin ? (
                            <div
                              className={`inline-flex items-center justify-center px-2 py-1 rounded-md text-tiny font-semibold ${LEVEL_STYLES.edit}`}
                              title={
                                u.role === "owner"
                    ...
                    <td className="px-2 py-2 border-b border-border-soft">
                      {isAdmin && !isRoleAdmin && (
                        <button
                          onClick={() => resetUserPermissions(u.id)}
                          className="p-1 rounded-md text-muted-soft hover:text-ink hover:bg-surface-sunk transition-colors"
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          <span className="text-tiny font-semibold text-muted uppercase tracking-wider">Legend:</span>
          {LEVELS.map((lv) => (
            <span
              key={lv}
              className={`inline-flex items-center px-2 py-0.5 rounded-md text-tiny font-semibold ${LEVEL_STYLES[lv]}`}
            >
              {t(`level_${lv}`)}
            </span>
          ))}
