      ...
      <div className="p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] border-separate border-spacing-0">
            <thead>
              <tr>
                <th className="text-left sticky left-0 bg-white z-10 py-2.5 pr-4 text-[10px] font-bold text-muted tracking-[0.1em] uppercase border-b border-border-soft">
                  User
                </th>
                {SECTIONS.map((s) => (
                  <th
                    key={s}
                    className="text-center px-2 py-2.5 text-[10px] font-bold text-muted tracking-[0.1em] uppercase border-b border-border-soft whitespace-nowrap"
                  >
                    {t(`section_${s}`)}
                  </th>
                ))}
                    ...
                    <td className="sticky left-0 bg-white py-2 pr-4 border-b border-border-soft">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-muted">{ROLES[u.role]?.label}</div>
                        </div>
                      </div>
                    </td>
                    {SECTIONS.map((s) => {
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          <span className="text-[11px] font-semibold text-muted uppercase tracking-wider">Legend:</span>
          {LEVELS.map((lv) => (
            <span
              key={lv}
