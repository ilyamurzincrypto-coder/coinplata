  ...
  return (
    <>
      <div className="pl-9 pr-card py-1.5 flex items-center gap-2 cursor-pointer hover:bg-surface-sunk border-t border-border-soft bg-surface-soft/50 transition-colors" onClick={() => setExpanded((v) => !v)}>
        ...
        <span className="text-caption tabular-nums tabular text-muted w-32 text-right whitespace-nowrap">
          {parent ? (
            <InlineBalanceEditor
              account={{
                code: parent.code,
        ...
        <span className="text-caption tabular-nums tabular font-semibold text-ink w-28 text-right whitespace-nowrap">{formatBase(dim.balanceInBase * displayMul, baseCurrency)}</span>
      </div>
