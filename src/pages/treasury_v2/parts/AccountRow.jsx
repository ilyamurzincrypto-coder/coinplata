  ...
  return (
    <>
      ...
      >
        ...
        <span className="tabular-nums text-tiny text-muted-soft w-12">{account.code}</span>
        <span className="flex-1 text-body-sm font-medium text-ink truncate">{account.name}</span>
        <span className="text-body-sm tabular-nums tabular text-ink-soft w-32 text-right whitespace-nowrap">
        ...
        <span className="text-body-sm tabular-nums tabular font-semibold text-ink w-28 text-right whitespace-nowrap">{formatBase(account.balanceInBase * displayMul, baseCurrency)}</span>
      </div>
      {expanded && (dims
        ? (
            <>
