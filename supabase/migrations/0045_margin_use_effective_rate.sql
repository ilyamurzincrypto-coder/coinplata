-- ============================================================================
-- CoinPlata · 0045_margin_use_effective_rate.sql
--
-- Баг: create_deal и update_deal при расчёте margin_usd брали ТОЛЬКО
-- глобальный pair (`pairs WHERE is_default`), игнорируя
-- office_rate_overrides из 0021.
--
-- Последствие: если у офиса свой override курса (например USD→TRY=45.5
-- при global=44.92), UI показывал profit от office rate, а БД писала
-- profit от global rate. Расхождение ~1.3% на каждой сделке этого
-- офиса. deals.profit_usd / fee_usd / referral_bonus оказывались
-- неверными для всех офисов с override.
--
-- Фикс: использовать public.effective_rate(office_id, from, to) — она
-- возвращает override > global fallback. Семантически: "market rate
-- по которому работает этот офис".
--
-- Сигнатуры функций сохранены (CREATE OR REPLACE) — клиентский код
-- не меняется.
-- ============================================================================
...
begin
  ...
  -- Margin: market rate берём из effective_rate(office, from, to) —
  -- учитывает per-office override. Это синхронно с UI.
...
-- ============================================================================
-- update_deal — те же изменения margin
-- ============================================================================
