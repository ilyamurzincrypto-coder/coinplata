-- 0003_update_deal.sql
-- Adds `update_deal` RPC so Edit Transaction persists to DB.
--
-- Стратегия: id сделки сохраняется, но legs/movements/obligations полностью
-- сносятся и пересоздаются (consistent with the frontend "rewrite" pattern —
-- см. buildMovementsFromTransaction + removeMovementsByRefId).
--
-- Безопасно, т.к. deal_legs.id и movement.id — derived, внешних ссылок на них
-- нет. Blockchain_txs ссылаются на matched_deal_id (bigint) — он сохраняется.
...
begin
  ...
  -- Полный teardown старых дочерних записей
  ...
  -- Пересчёт margin/fee/profit (логика идентична create_deal)
  ...
  update public.deals set
    ...
    checking_started_at = case when v_final_status = 'checking' then v_now else null end,
    checking_by = case when v_final_status = 'checking' then v_manager_id else null end,
    in_planned_at = v_now,
    in_actual_amount = case when v_final_status = 'completed' and p_in_account_id is not null then p_amount_in else 0 end,
    in_completed_at = case when v_final_status = 'completed' and p_in_account_id is not null then v_now else null end,
    updated_at = v_now
  where id = p_deal_id;

  -- Пересоздаём IN movement
  ...
  -- Пересоздаём legs + их movements/obligations
  for v_leg in select * from jsonb_array_elements(p_legs) loop
    v_is_internal := false;
    v_leg_office := null;
    ...
    if (v_leg->>'account_id') is not null and (v_leg->>'account_id') <> '' then
      if v_available >= v_leg_planned then
        ...
        insert into public.obligations (
          ...
          'Auto-created on edit: insufficient balance',
          v_manager_id
        );
