-- ============================================================================
-- CoinPlata · 0013_update_deal_v2.sql
-- Фикс: update_deal теперь принимает planned_at + deferred_in + per-leg pay_now
-- (как и create_deal после 0009). Раньше редактирование pending/partial
-- сделки сбрасывало эти поля на defaults.
-- ============================================================================
...
begin
  ...
  -- Teardown: полностью сносим дочерние записи, пересоздаём заново.
  ...
  if p_deferred_in then
    insert into public.obligations (
      ...
      'they_owe',
      'Deferred pay-in (edited): client will pay later',
      v_manager_id
    );
      ...
      if v_use_manual_payout then
        ...
        if v_pay_now > 0 then
          insert into public.account_movements (
            ...
            'Deal #' || p_deal_id || ' · leg ' || (v_idx+1) || ' (edited)',
            v_manager_id
          );
        end if;
        if v_remaining > 0 then
          insert into public.obligations (
            ...
            case when v_pay_now = 0
              then 'Deferred payout (edited)'
              else 'Partial (edited): paid ' || v_pay_now
            end,
            v_manager_id
          );
          v_has_obligation := true;
