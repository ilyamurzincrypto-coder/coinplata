...
**Акценты по слайсам (утверждены):**
- **1.5.e:** офис — required (леджер-счёт кассы несёт `office_id`); валютный гейт = только валюты с позицией в Капитале; человеческий отказ «Сначала добавьте валюту X — у неё нет балансового счёта в Капитале» + кнопка в мастер (акцент №2); отдельный поток (общий `ChartAccountModal` не ограничиваем — эксперту нужны revenue/expense без позиций).
- **1.5.f:** чекбоксы счетов при заведении клиента — **только валюты с позицией** (тот же гейт и текст, что 1.5.e); клиентские счета сразу несут знаковый канон (+raw: держит→плюс, должник→минус **красным**) в списке клиентов и карточке.
  - **Карточка клиента (уточнение):** ВСЕ открытые валюты строками, включая нулевые («счёт открыт, пуст» — эксперт видит, что счёт есть). Порядок **фиксированный**: базовая валюта первой, дальше по коду/алфавиту — НЕ по остатку (кассир держит глазную привычку, бухгалтер читает позиционно). Ненулевое выделять **весом** (жирнее), не позицией. Валюта НЕ в списке открытых, но с ненулевым остатком (легаси) — показать с пометкой «не в списке открытых» (деньги не прячем ни при какой настройке). Слово «измерение» — нигде.
- **1.5.g:** липкость — тест «клик по фону при заполненной форме → форма на месте, данные целы» на **ВСЁМ инвентаре модалок** (вкл. уже собранные CurrencyWizard, ChartAccountModal, CashboxWizard), не только новых; Esc/крестик + подтверждение при заполненных полях.
...
- **Лоро клиентов: общий счёт на валюту с client-измерением или персональный счёт на клиента — как в твоём плане счетов?** (Сейчас, модель A, — общий + измерение, совместимо с живым `create_deal_v2`. Персональный счёт-на-клиента = развилка B = Этап 2: требует переработки деал-тракта под новый план счетов. Твоё слово на заведении нового бизнеса определит Этап 2.)
...
**Слайс 1.5.d — ВЫПОЛНЕНО (2026-07-27):** Панель «Ностро − Лоро = Капитал ✓» + офисное измерение. Открытие: **AssetsTab уже группирует Ностро Office→Currency→Account** (`assetsByOfficeCurrency`) — коррекция «Ностро по офисам» уже была в проде. Новое: селектор `capitalByCurrency` (v2selectors) + `parts/CapitalPanel.jsx` (валюты строками; колонки Ностро·Лоро·Капитал + ✓-чекбокс; «складывается на глазах»; знаки CP PAY **дословно** — Лоро=+raw: клиент держит→+, должник→−; Капитал=Ностро−Лоро; display-слой, dr/cr не тронуты). Свойство сходимости Σ(Ностро−Лоро)=Активы−Пассивы=Капитал+PnL → «Σ ✓» против `balanceCheckTotals.equity`. Вживлён сверху `EquityTab`. Тест знаков `capitalByCurrency.test.js` (5/5), скрин предъявлен (Лоро −€8k должник, Σ $795,200 ✓). 752 теста, build зелёные.

**Слайс 1.5.e — ВЫПОЛНЕНО (2026-07-27):** `parts/CashboxWizard.jsx` (отдельный поток на общем `ui/Modal.jsx`) — Касса/Банк/Кошелёк, **офис required** (леджер-счёт несёт `office_id` через `rpcCreateLedgerAccount`), **валютный гейт** = только валюты с позицией в Капитале (`ctx.accounts` subtype='position'), **человеческий отказ** при отсутствии позиций («Нет ни одной валюты с балансовым счётом… Сначала добавьте валюту») + кнопка в `CurrencyWizard` (акцент №2), + ссылка «Добавить валюту» всегда. Кнопка «Касса» в `AssetsTab` рядом с общим «+ Счёт» (общий `ChartAccountModal` не тронут — эксперту нужны счета без позиций). Скрины обоих состояний предъявлены. 752 теста, build зелёные.

**Слайс 1.5.f — БЭКЕНД ВЫПОЛНЕН (2026-07-27, prod):** модель A (общий customer_liab на валюту + client-измерение; персональный счёт-на-клиента = развилка B = Этап 2, в README-вопросах). Схема: `public.clients.currencies text[]` (открытые клиенту валюты) + RPC `public.create_client_with_liab(nickname,full_name,telegram,tag,note,referrer_id,currencies[])` — атомарно клиент + ensure общий `customer_liab` на каждую валюту (`client_dim_required=true`, код `CL`+сегмент, эксперт перекодирует); **гейт валют ВНУТРИ RPC** (позиция обязательна, человеческий отказ); идемпотентно (повтор — не дубль; тот же nickname с другим telegram → raise). Писатель `rpcCreateClientWithLiab` (newLedger). Проверено на TEST-данных (подчищены): happy · идемпотентность (2 вызова→1 клиент) · гейт (USD без позиции→отказ) · конфликт telegram→raise.

**Слайс 1.5.f — КАРТОЧКА ВЫПОЛНЕНА (2026-07-27):** селектор `clientLedgerRows` (v2selectors, тест 5/5) + секция «Расчётные счета» в `ClientProfileModal` (вязка: `loadClients` маппит `currencies`, `ListTab` пробрасывает ledger-ctx через `useLedger`). Правила эксперта в рендере (скрин): ВСЕ открытые валюты строками вкл. нулевые; **фикс-порядок база→код** (не по остатку); знак **+raw** (держит→+, должник→−**красным**); ненулевое — **весом**; не-открытая-с-остатком → бейдж **«не в списке открытых»** (деньги не прячем); клик по строке → фильтр движений по валюте; слово «измерение» — нигде. 757 тестов, build зелёные.

**Слайс 1.5.f — AddClientModal ВЫПОЛНЕН (2026-07-27):** `AddClientModal` — секция «Открыть счета в валютах» (чипы-чекбоксы, **гейт по позициям** через `positionCurrencies` из `ListTab`/`useLedger`) + **submitting-guard** (busy: «Сохраняю…» + disabled, дабл-клик не плодит дубль) + маршрут через атомарный `rpcCreateClientWithLiab` (вместо старого `insertClient`). Скрин предъявлен (USD/TRY/USDT-TRC20 отмечены). 757 тестов, build зелёные. **Осталось (мелочь 1.5.f):** список клиентов — агрегат +raw. Слайс 1.5.f по сути закрыт (бэкенд + карточка + заведение).

**Слайс 1.5.g — ВЫПОЛНЕНО (2026-07-27):** липкость системно через общий `ui/Modal.jsx` — клик по фону **НЕ закрывает** форму (данные целы), Esc/крестик закрывают через `requestClose` с **подтверждением при `dirty`**. Тесты `Modal.test.jsx` (10/10): поведение (фон-не-закрывает+данные-целы, Esc, крестик, dirty-отмена, dirty-принятие) + **прогон инвентаря модалок** (CurrencyWizard/CashboxWizard/ChartAccountModal/AddClientModal/ClientProfileModal — статически проверено, что все импортируют общий `Modal` → липкость получают все). Полный прогон 767 тестов зелёный (правка Modal ничего не сломала), build чист.

---

**Слайс 1.5.f — ГЕЙТ ДВУХСТУПЕНЧАТЫЙ (2026-07-27, правило эксперта):** валюта открываема клиенту только если **позиция в Капитале И хотя бы один наш актив** (касса/банк/кошелёк) в валюте — лоро без покрытия активом нельзя. Серверно: `create_client_with_liab` проверяет обе ступени (проверено: позиция без актива → «Нет кассы/счёта в X — сначала откройте»; позиция+актив → проходит). Фронт: `AddClientModal` берёт `ledgerCtx`, чекбокс валюты без актива **заблокирован** с подписью «Нет кассы/счёта в X — откройте» + кнопка → инлайн `CashboxWizard`. Скрин предъявлен.

## ЭТАП 1.5 — ПОТОК c→g ЗАКРЫТ ✅ (2026-07-27)
Схема (a·b) + мастер валют (c) + панель Ностро−Лоро=Капитал (d) + заведение касс/кошельков (e) + клиенты-лоро атомарно/карточка/заведение (f) + липкость всех форм (g). Скрины предъявлены на c·d·e·f-карточка·f-AddClientModal. Осталось: список клиентов агрегат +raw (мелочь). **Финальные ворота 1.5 — сольный прогон эксперта-владельца** (валюта→позиция→касса→клиент→сделка).
