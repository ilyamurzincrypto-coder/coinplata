...
> Проверь точные пути/имена хуков (`useOffices`, `useCurrencies`) — поправь под
> существующие экспорты, если отличаются.

Внутри компонента, рядом с прочими `useState`:

```js
  const { applyOfficeOverrideLocal, setSpecialRatesSnapshot } = useRates();
...
```js
  const handleApplyText = async () => {
    ...
    try {
      ...
      // Спец-курсы → снимок (информационно)
      const stamped = (textParsed.special || []).map((s) => ({ ...s, importedAt: new Date().toISOString() }));
      setSpecialRatesSnapshot(stamped);
      logAudit({
        action: "update",
        entity: "rates",
        entityId: "bulk",
        summary: `morning-import: ${textParsed.updates.length} якорей, ${stamped.length} спец, ${textParsed.skipped.length} пропущено`,
      });
      handleClose();
    } finally {
      setSubmitting(false);
...
> Импортировать `rpcUpsertOfficeRate` из `../lib/supabaseWrite` (проверь, что он
> ещё не импортирован в файле — если нет, добавь к существующему импорту).
