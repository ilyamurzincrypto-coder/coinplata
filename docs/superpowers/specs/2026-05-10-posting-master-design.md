...
7. For each line: if the account's `client_dim_required` is true, `client_id` must be present (and must reference an existing client); same for `partner_dim_required` / `partner_id`.
...
- Backdating allowed (no period-close concept exists in v2 yet). `p_effective_date` defaults to `now()`.
- If `ledger.transactions.source_kind` has a CHECK constraint, the migration adds `'manual'` to it.
...
- **Lines table:** each row = `[ account picker | Dr amount | Cr amount | counterparty picker | × remove ]`.
  - A line is either Dr or Cr, not both — entering a value in one column clears the other.
  - The counterparty picker (client / partner) is shown and required only when the selected account's `client_dim_required` / `partner_dim_required` flag is set; hidden otherwise.
  - "+ Добавить строку" button; minimum 2 lines (remove disabled at 2).
- **Account picker (`src/pages/treasury_v2/parts/AccountPicker.jsx`, new):** searchable dropdown over `useLedger().accounts` filtered to `currency === selectedCurrency && active`, showing `code · name · subtype`. A soft amber chip ("обычно ведётся автоматически") on system-driven subtypes — `customer_liab`, `partner_liab`, `clearing`, `fx_clearing`, `crypto_input`, `crypto_output` — informational only, does not block selection. (`AccountSelect.jsx` in the cashier is for legacy `public.accounts`; this is a fresh small component for `ledger.accounts`.)
...
- **Submit** — disabled until: balanced (Δ=0), reason non-empty, ≥1 dr and ≥1 cr line, every line valid (account chosen, positive amount, required counterparty present). On click → `rpcCreateManualEntryV2(...)`:
...
New `trv2_pm_*` keys in `src/i18n/translations.jsx` (en / ru / tr): tab label, "add line" / "remove line", Dr / Cr column headers, account, counterparty, reason, description, currency, effective date, balance Δ readout, submit button, the system-driven-subtype warning chip, success toast, error messages (auth denied / unknown currency / does-not-balance / generic), "Сторнировать" + the reverse-reason modal labels. Plus `trv2_journal_type_manual` and add `'manual'` to the `TYPES` array in `JournalTab.jsx` so the Журнал type filter has a "Manual entries" button.
