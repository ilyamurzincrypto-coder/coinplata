...
- **Откат/revert/reset/force-push — ТОЛЬКО по явной команде владельца со словом «откати».** Критика внешнего вида («выглядит плохо», «не работает как задумано») = задача на ПРАВКИ по списку, НЕ разрешение откатывать/удалять работу. Спорные места — вопросом владельцу, не действием.
